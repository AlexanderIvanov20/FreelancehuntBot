...
bot.start((ctx) => {
  logger.log(
    "Start command. Create user session and add he/she in a database."
  );
  ...
  ctx.reply(
      ...
      "Перед началом отслеживания проектов, _нажмите на кнопку ниже_.",
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: "Выбрать категории", callback_data: "selectSkills" }],
        ],
      },
    }
  );
...
bot.on("callback_query", (ctx) => {
  ...
  if (ctx.callbackQuery.data === "selectSkills") {
    ...
    intervalForSendingProjects = setInterval(() => {
      ...
      ctx.session.tracker
        .sendProject(ctx.session.selectedSkills)
        .then((newProjects) => {
          newProjects.forEach((element) => {
            ...
            if (!ctx.session.showedProject.includes(element.projectId)) {
              ...
              const realAmount =
                element.amount !== -1 ? element.amount : "Договорная";
